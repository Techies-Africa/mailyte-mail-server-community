...
class CertificateManager:
    ...
    def provision_certificate(self, domain, domain_list=None):
        ...
        try:
            ...
            self._notify_webhook('certificate_provision_failed', {
                'domain': domain,
                'error': str(e)
            })
            return False
    ...
    def _provision_letsencrypt(self, domain, domain_list=None):
        ...
        cmd = [
            ...
            '--non-interactive',
            '--agree-tos',
            '--email', self.config['CERT_ACME_EMAIL'],
            '--cert-name', domain,
        ]
        ...
        try:
            ...
            # Notify webhook
            self._notify_webhook('certificate_provisioned', {
                'domain': domain,
    ...
    def renew_certificates(self):
            ...
            try:
                if self._needs_renewal(cert_info['expiry']):
                    ...
                    if self._renew_certificate(cert_name):
                        renewed_count += 1
                        self._notify_webhook('certificate_renewed', {
                            'domain': cert_name,
        ...
        if failed_renewals:
            self._notify_webhook('certificate_renewal_failed', {
                'failed_certificates': failed_renewals
            })
            ...
            for cert_name, cert_info in certificates.items():
                ...
                if expiry:
                    ...
                    if days_until_expiry <= 7:
                        health_status['issues'].append(f"Certificate {cert_name} expires in {days_until_expiry} days")
...
def provision_certificate():
    ...
    if success:
        return jsonify({
            'status': 'success',
            'message': f'Certificate provisioned for {domain}'
        })
    else:
        return jsonify({
            'status': 'error',
