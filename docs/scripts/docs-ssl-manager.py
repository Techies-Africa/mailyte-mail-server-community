...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/var/log/docs-ssl/ssl-manager.log"), logging.StreamHandler()],
)
...
class DocsSSLManager:
            ...
            if result.returncode == 0:
                ...
                if "=" in expiry_line:
                    ...
                    if days_until_expiry <= self.renewal_days:
                        logger.info(
                            f"Certificate expires in {days_until_expiry} days, needs renewal"
                        )
                        return False
                    else:
    ...
    def _copy_certificates(self):
        ...
        try:
            ...
            subprocess.run(
                ["cp", f"{self.cert_path}/fullchain.pem", f"{self.nginx_cert_path}/docs.crt"],
                check=True,
            )

            # Copy private key
            subprocess.run(
                ["cp", f"{self.cert_path}/privkey.pem", f"{self.nginx_cert_path}/docs.key"],
                check=True,
            )
...
"""
    ...
    def _send_webhook_notification(self, event_type, success, error_message=None):
        ...
        try:
            ...
            response = requests.post(self.webhook_url, data=payload, headers=headers, timeout=30)
...
"""
    ...
    def run_continuous_monitoring(self):
        ...
        logger.info(
            f"Starting continuous SSL monitoring (check every {self.check_interval} seconds)"
        )
