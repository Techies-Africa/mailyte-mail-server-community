...
| ActiveSync (Enterprise Edition) | 8084→80 | `activesync` | Z-Push 2.7.4 (Exchange ActiveSync) over Dovecot IMAP |
| Monitoring (Enterprise Edition) | 8085 | `monitoring` | Health checks, container restarts via docker-proxy, alerting |
...
| Dashboard (Enterprise Edition) | 8088 | `dashboard` | Server-rendered stats dashboard |
| Archiver (Enterprise Edition) | 8089→8083 | `archiver` | age-encrypted archival of every message to S3; retention, legal hold |
| Queue Manager (Enterprise Edition) | 8090 | `queue_manager` | Postfix queue inspection and control via the shared spool |
| RAG / AI Search (Enterprise Edition) | 8091→8090 | `rag` | Semantic email search with Qdrant vector embeddings |
| Storage Usage (Enterprise Edition) | 8092 | `storage_usage` | Quota accounting via Dovecot IMAP QUOTA; alerts |
| Encryption (Enterprise Edition) | 8093→8084 | `encryption` | PGP/S-MIME key management, WKD, message crypto |
| Delivery Optimizer (Enterprise Edition) | 8094→8088 | `delivery_optimizer` | ISP throttling, IP warming, bounce/FBL processing, reputation |
| [Templates](templates.md) | 8095→8089 | `templates` | Jinja2 email templates with versioning and stats |
| URL Protection (Enterprise Edition) | 8096→8090 | `url_protection` | Safe Links: rewrite + time-of-click verification |
| OAuth (Enterprise Edition) | 8097→8091 | `oauth` | OAuth 2.0 / XOAUTH2 authorization server |
| JMAP (Enterprise Edition) | 8098 | `jmap` | JMAP Core + Mail (RFC 8620/8621) over Dovecot |
| Migration (Enterprise Edition) | 8099 | `migration` | IMAP-to-IMAP mailbox import/export with delta sync |
...
| CalDAV (Enterprise Edition) | 8101 | `caldav` | Calendar/contacts management API over Radicale |
