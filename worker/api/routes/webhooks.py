...
async def list_endpoints(request: Request, page: int = Query(1), per_page: int = Query(50)):
    ...
    try:
        ...
        return JSONResponse(
            content=create_api_response("error", "Failed to retrieve webhook endpoints"),
            status_code=500,
        )
    finally:
        conn.close()
...
async def create_endpoint(request: Request):
    ...
    if not data or not data.get("url"):
        return JSONResponse(
            content=create_api_response("error", "url is required"), status_code=400
        )

    url = data["url"].strip()
    if not url.startswith(("http://", "https://")):
        return JSONResponse(
            content=create_api_response("error", "url must start with http:// or https://"),
            status_code=400,
        )
    ...
    if event_types is not None and not isinstance(event_types, list):
        return JSONResponse(
            content=create_api_response("error", "event_types must be a list or null"),
            status_code=400,
        )
    ...
    try:
        ...
        return JSONResponse(
            content=create_api_response("error", "Failed to create webhook endpoint"),
            status_code=500,
        )
    finally:
        conn.close()
...
async def get_endpoint(endpoint_id: str, request: Request):
    ...
    try:
        ...
        if not row:
            return JSONResponse(
                content=create_api_response("error", "Webhook endpoint not found"), status_code=404
            )
        ...
        return JSONResponse(
            content=create_api_response("error", "Failed to retrieve webhook endpoint"),
            status_code=500,
        )
    finally:
        conn.close()
...
async def update_endpoint(endpoint_id: str, request: Request):
    ...
    try:
        ...
        if not updates:
            cursor.close()
            return JSONResponse(
                content=create_api_response("error", "No valid fields to update"), status_code=400
            )
        ...
        return JSONResponse(
            content=create_api_response("error", "Failed to update webhook endpoint"),
            status_code=500,
        )
    finally:
        conn.close()
...
async def delete_endpoint(endpoint_id: str, request: Request):
    ...
    try:
        ...
        return JSONResponse(
            content=create_api_response("error", "Failed to delete webhook endpoint"),
            status_code=500,
        )
    finally:
        conn.close()
...
async def test_endpoint(endpoint_id: str, request: Request):
    ...
    try:
        ...
        if not row:
            return JSONResponse(
                content=create_api_response("error", "Webhook endpoint not found or inactive"),
                status_code=404,
            )
        if org_id and row.get("organization_id") != org_id:
            return JSONResponse(
        ...
        return JSONResponse(
            content=create_api_response("error", "Failed to test webhook endpoint"), status_code=500
        )
    finally:
        conn.close()
...
async def list_deliveries(
    ...
    try:
        ...
        return JSONResponse(
            content=create_api_response("error", "Failed to retrieve delivery log"), status_code=500
        )
    finally:
        conn.close()
