...
class MailboxCreate(BaseModel):
    email: str = Field(..., description="Full email address for the new mailbox", example="user@example.com")
    password: str = Field(..., description="Password for IMAP/SMTP authentication (will be bcrypt-hashed)", example="SecureP@ss123!")
    name: Optional[str] = Field(None, description="Display name of the mailbox owner", example="John Doe")
    domain_id: str = Field(..., description="ULID of the domain this mailbox belongs to", example="01ARZ3NDEKTSV4RRFFQ69G5FAV")
    storage_quota: int = Field(5368709120, description="Storage quota in bytes (default 5GB)", example=5368709120)


class MailboxUpdate(BaseModel):
    name: Optional[str] = Field(None, description="Updated display name")
    status: Optional[str] = Field(None, description="Account status: active, inactive, or suspended", example="active")
    ...
    forward_destination: Optional[str] = Field(None, description="Forwarding destination email address")
...
async def get_email_account(account_id: str):
    ...
    try:
        ...
        return JSONResponse(content=create_api_response(
            'error',
            'Failed to retrieve email account'
        ), status_code=500)
    finally:
        session.close()
...
async def create_email_account(request: Request):
    ...
    try:
        ...
        existing_account = session.query(EmailAccount).filter(EmailAccount.email.ilike(data['email'])).first()
        if existing_account:
            return JSONResponse(content=create_api_response(
                'error',
                f'Email account {data["email"]} already exists'
            ), status_code=409)

        # Check if external_id already exists (if provided)
        if data.get('external_id'):
            existing_external = session.query(EmailAccount).filter_by(external_id=data['external_id']).first()
            if existing_external:
                return JSONResponse(content=create_api_response(
                    'error',
                    'Email account with this external_id already exists'
                ), status_code=409)
        ...
        if account_count >= domain.max_users:
            return JSONResponse(content=create_api_response(
                'error',
                f'Domain has reached maximum user limit ({domain.max_users})'
            ), status_code=400)
        ...
        dispatch_event(
            ...
            data={"account_id": account.id, "email": account.email, "domain_id": account.domain_id, "organization_id": account.organization_id},
...
async def update_email_account(account_id: str, request: Request):
    ...
    try:
        ...
        dispatch_event(
            ...
            data={"account_id": account.id, "email": account.email, "organization_id": account.organization_id, "updated_fields": [k for k in data.keys() if k != 'password']},
            org_id=account.organization_id,
            source_service="api",
        )
...
async def delete_email_account(account_id: str):
    ...
    try:
        ...
        dispatch_event(
            ...
            data={"account_id": account_id_val, "email": account_email_val, "organization_id": account_org_id_val},
...
@router.get('/email-accounts/{account_id}/quotas',
...
async def get_account_quotas(account_id: str):
    ...
    try:
        ...
        quota_info = {
            ...
            'last_storage_calculation': account.last_storage_calculation.isoformat() if account.last_storage_calculation else None,
            'over_threshold': account.is_storage_over_threshold()
        }

        return create_api_response(
            'success',
            'Account quota information retrieved successfully',
            quota_info
        )
...
@router.put('/email-accounts/{account_id}/quotas',
...
async def add_mailbox(request: Request):
    ...
    if not re.match(r'^[a-zA-Z0-9._%+-]+$', local_part):
        return JSONResponse(content=create_api_response(
            'error', 'Invalid local_part: only letters, numbers, dots, hyphens, underscores allowed'
        ), status_code=400)
    ...
    if 'email' in data and data['email']:
        ...
        if provided_email != email:
            return JSONResponse(content=create_api_response(
                'error', f'Email field ({provided_email}) must match local_part@domain ({email})'
            ), status_code=400)
    ...
    try:
        ...
        if not cursor.fetchone():
            return JSONResponse(content=create_api_response(
                'error',
                f"Domain {data['domain']} not found or inactive"
            ), status_code=400)
        ...
        if not domain_row:
            return JSONResponse(content=create_api_response(
                'error', f"Domain {data['domain']} not found"
            ), status_code=400)
        ...
        return create_api_response(
            'success',
            f'Mailbox {email} created successfully',
    ...
    try:
        ...
        for mailbox in data['items']:
            ...
            for key, value in data['attr'].items():
                ...
                elif key in ['name', 'quota', 'active', 'force_pw_update',
                           'tls_enforce_in', 'tls_enforce_out', 'quarantine_notification',
                           'quarantine_category', 'rl_value', 'rl_frame']:
                    update_fields.append(f"{key} = %s")
            ...
            if update_fields:
                ...
                if cursor.rowcount > 0:
                    dispatch_event(
                        Events.MAILBOX_UPDATED,
                        data={"email": mailbox, "updated_fields": [k for k in data['attr'].keys() if k != 'password']},
                        source_service="api",
                    )
                    results.append({
                        'mailbox': mailbox,
...
async def delete_mailbox(request: Request):
    ...
    if not data or not isinstance(data, list):
        return JSONResponse(content=create_api_response(
            'error',
            'Invalid request format - array of mailboxes expected'
        ), status_code=400)
    ...
    try:
        ...
        for mailbox in data:
            try:
                ...
                cursor.execute("DELETE FROM aliases WHERE destination LIKE %s OR source = %s",
                             (f"%{mailbox}%", mailbox))
                ...
                if user_deleted > 0:
                    ...
                    dispatch_event(
                        Events.MAILBOX_DELETED,
                        data={"email": mailbox, "aliases_deleted": aliases_deleted, "messages_deleted": messages_deleted},
                        source_service="api",
                    )
                    results.append({
                        'mailbox': mailbox,
...
async def edit_mailbox_quota(request: Request):
    ...
    if not data or 'mailbox' not in data or 'quota' not in data:
        return JSONResponse(content=create_api_response(
            'error',
            'Missing required fields: mailbox, quota'
        ), status_code=400)
    ...
    try:
        ...
        if cursor.rowcount > 0:
            return create_api_response(
                'success',
                f"Quota updated for {data['mailbox']}"
                        )
        else:
            return JSONResponse(content=create_api_response(
                'error',
...
async def get_mailbox_stats(mailbox: str):
    ...
    try:
        ...
        return JSONResponse(content=create_api_response(
            'error',
            'Failed to retrieve mailbox statistics'
        ), status_code=500)
    finally:
        conn.close()
