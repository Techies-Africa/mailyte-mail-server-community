...
class EmailProcessor:
    ...
    def __init__(self):
        ...
        self.sensitive_headers = {
            'X-Originating-IP', 'Received', 'X-Mailer', 'User-Agent',
            'X-Forwarded-For', 'X-Real-IP', 'X-Client-IP', 'X-Source-IP',
            'X-Remote-IP', 'X-Cluster-Client-IP', 'CF-Connecting-IP'
        }

        # Headers that are safe to include
        self.safe_headers = {
            'Message-ID', 'Subject', 'From', 'To', 'Cc', 'Bcc',
            'Date', 'Reply-To', 'Return-Path', 'Content-Type',
            'Content-Transfer-Encoding', 'MIME-Version', 'In-Reply-To',
            'References', 'Thread-Topic', 'Thread-Index'
        }
    ...
    def extract_attachments(self, eml_content: str, include_content: bool = False) -> List[Dict[str, Any]]:
        """
                        ...
                        try:
                            ...
                            if payload:
                                ...
                                if len(payload) > self.max_attachment_size:
                                    logger.warning(f"Attachment {filename} too large: {len(payload)} bytes")
                                    continue
                                ...
                                if (include_content and 
                                    file_ext in self.allowed_attachment_types and
                                    len(payload) <= self.max_attachment_size):

                                    attachment_data['content_base64'] = base64.b64encode(payload).decode('utf-8')
    ...
    def _check_sensitive_content(self, metadata: Dict[str, Any]) -> bool:
        ...
        sensitive_indicators = [
            ...
            'confidential', 'private', 'secure', 'login',
            'financial', 'medical', 'health'
        ]
