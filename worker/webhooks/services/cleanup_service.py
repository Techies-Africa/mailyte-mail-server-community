...
class WebhookCleanupService:
    ...
    def _cleanup_successful_deliveries(self, session) -> int:
        ...
        while True:
            ...
            records_to_delete = session.query(WebhookDeliveryLog.id).filter(
            ...
            deleted_count = session.query(WebhookDeliveryLog).filter(
                WebhookDeliveryLog.id.in_(record_ids)
