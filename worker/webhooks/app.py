...
class WebhookQueueManager:
    ...
    def _webhook_worker(self):
        ...
        while self.running:
            try:
                ...
                with LogTimer(log_performance, f"webhook_processing_{event_data.get('event', 'unknown')}"):
                ...
                if result['success']:
                    ...
                    logger.debug(f"Webhook processed successfully in {processing_time:.3f}s by {worker_name}")
                else:
                    self.error_count += 1
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            'success_rate': (self.processed_count / total_processed * 100) if total_processed > 0 else 100.0,
...
def track_email_storage_usage(webhook_payload, metadata, attachments):
    ...
    logger.info(f"Storage usage tracked for org {organization_id}: Email size: {email_size}, Attachments: {total_attachment_size}, Total: {total_storage_used}")
...
async def handle_imap_event(request: Request):
    ...
    try:
        ...
        logger.info(f"IMAP webhook queued: {data.get('action', 'unknown')} by {data.get('user', 'unknown')}")
...
async def handle_pop3_event(request: Request):
    ...
    try:
        ...
        logger.info(f"POP3 webhook queued: {data.get('action', 'unknown')} by {data.get('user', 'unknown')}")
...
async def update_cleanup_config(request: Request):
    ...
    try:
        ...
        return JSONResponse({
            ...
            "new_config": cleanup_service.config
        }, status_code=200)
    except Exception as e:
...
if __name__ == '__main__':
    ...
    cleanup_config = {
        ...
        'auto_delete_successful': os.getenv('WEBHOOK_AUTO_DELETE_SUCCESSFUL', 'true').lower() == 'true',
