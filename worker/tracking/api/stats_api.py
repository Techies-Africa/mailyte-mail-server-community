...
async def inject_tracking(request: Request):
    ...
    try:
        ...
        if enable_click_tracking:
            try:
                modified_content, links_rewritten = (
                    request.app.tracking_service.rewrite_links_for_tracking(
                        modified_content, email_id, recipient, tenant_id, domain_id
                    )
                )
                logger.debug(
                    f"Rewritten {links_rewritten} links for click tracking in email {email_id}"
                )
            except Exception as e:
                logger.error(f"Failed to rewrite links for tracking: {e}")
                return JSONResponse(
                    {"error": "Failed to rewrite links for tracking"}, status_code=500
                )
        ...
        logger.info(
            f"Tracking successfully injected for email {email_id} "
        ...
        return JSONResponse(
            {"error": "Internal server error during tracking injection"}, status_code=500
        )
