...
class EnterpriseWebhookService:
    ...
    def get_webhooks_for_event(self, event_type: str, service_type: str, 
                              tenant_id: str = None, domain_id: str = None) -> List[Dict[str, Any]]:
        """
        ...
        if (cache_key in self._webhook_cache and 
        ...
        try:
            ...
            query = session.query(WebhookURL).filter(
            ...
            query = query.filter(
                text("JSON_CONTAINS(event_types, :event_type)")
            ).params(event_type=f'"{event_type}"')
            
            # Filter by service type (JSON contains)
            query = query.filter(
                text("JSON_CONTAINS(service_types, :service_type)")
    ...
    def send_tracking_webhook(self, event_type: str, tracking_data: Dict[str, str], 
                            request_info: Dict[str, Any], additional_data: Dict[str, Any] = None):
        """
        ...
        # Create base payload
        payload = self._create_tracking_payload(event_type, tracking_data, request_info, additional_data)
    ...
    def send_smtp_webhook(self, event_type: str, smtp_data: Dict[str, Any]):
        ...
        webhooks = self.get_webhooks_for_event(
            f'smtp.{event_type}',
            'postfix',
            smtp_data.get('tenant_id'),
            smtp_data.get('domain_id')
        )
    ...
    def _create_tracking_payload(self, event_type: str, tracking_data: Dict[str, str], 
                               request_info: Dict[str, Any], additional_data: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    def _send_to_multiple_webhooks(self, webhooks: List[Dict[str, Any]], 
                                 payload: Dict[str, Any], event_identifier: str):
        ...
        logger.info(f"Webhook delivery for {event_identifier}: "
                   f"{successful_deliveries} successful, {failed_deliveries} failed")
    
    def _send_single_webhook(self, webhook: Dict[str, Any], payload: Dict[str, Any], 
                           event_identifier: str) -> bool:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                headers = {
                    ...
                    'X-Webhook-Signature': self._generate_signature(encrypted_payload, webhook['webhook_secret']),
                ...
                if webhook.get('auth_type') and webhook.get('auth_credentials'):
                    self._add_authentication(headers, webhook['auth_type'], webhook['auth_credentials'])
                ...
                if response.status_code in [200, 201, 202]:
                    ...
                    logger.warning(f"Webhook delivery failed to {webhook['name']}: "
        ...
        logger.error(f"Webhook delivery failed to {webhook['name']} after {max_retries + 1} attempts")
        return False
    ...
    def get_webhook_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for webhook in webhooks:
                ...
                is_healthy = (success_rate >= 90.0 and 
                            webhook.last_success and 
                            (datetime.utcnow() - webhook.last_success).total_seconds() < 3600)
