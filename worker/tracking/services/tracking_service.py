...
class TrackingService:
    ...
    def create_tracking_pixel_url(self, tracking_id: str, organization_id: str) -> str:
        ...
        domain = tracking_config.get(
            "tracking_domain", os.getenv("TRACKING_DOMAIN", "yourdomain.com")
        )
        subdomain = tracking_config.get(
        ...
        try:
            ...
            if organization_id and tracking_id:
                for link in soup.find_all("a", href=True):
                    ...
                    # Create tracking URL
                    tracking_url = self.create_click_tracking_url(
                        original_url, tracking_id, organization_id
                    )
                    ...
                    tracking_data["click_urls"].append(
                        {"original_url": original_url, "tracking_url": tracking_url}
                    )
    ...
    def _should_skip_url(self, url: str, organization_id: str) -> bool:
        ...
        tracking_config = self.config.get_organization_config(organization_id)
        tracking_domain = tracking_config.get(
            "tracking_domain", os.getenv("TRACKING_DOMAIN", "yourdomain.com")
        )
...
class TrackingService:
    ...
    def rewrite_links_for_tracking(
        self, html_content: str, email_id: str, recipient: str, tenant_id: str, domain_id: str
    ) -> tuple[str, int]:
        """
        ...
        def replace_link(match):
            ...
            tracking_url = self.create_click_tracking_url(
                original_url, email_id, recipient, tenant_id, domain_id
            )
            links_rewritten += 1
