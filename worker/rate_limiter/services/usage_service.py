...
class RateLimitUsageService:
    ...
    def get_current_usage(
        ...
        try:
            ...
            usage_counts = self.cache_service.get_current_counts(entity_type, identifier, direction)

            # Check if we got valid data from cache
            if usage_counts and any(count > 0 for count in usage_counts.values()):
                self.cache_hits += 1
            ...
            logger.debug(
                f"Cache miss, falling back to database for {entity_type}:{identifier}:{direction}"
            )
    ...
    def increment_usage(
        ...
        try:
            ...
            if success:
                logger.debug(
                    f"Usage incremented for {entity_type}:{identifier}:{direction} by {amount}"
                )
            else:
                self.errors += 1
                logger.error(
                    f"Failed to increment usage for {entity_type}:{identifier}:{direction}"
                )
    ...
    def reset_usage_counters(self, entity_type: str, identifier: str, direction: str) -> bool:
        ...
        try:
            ...
            if self.cache_service.redis_client:
                cache_reset = self.cache_service.reset_counters(entity_type, identifier, direction)
    ...
    def get_quota_status(
        ...
        try:
            ...
            hourly_remaining = max(
                0, config_rule.hourly_limit - usage_counts.get("hourly_count", 0)
            )
            daily_remaining = max(0, config_rule.daily_limit - usage_counts.get("daily_count", 0))
            monthly_remaining = max(
                0, config_rule.monthly_limit - usage_counts.get("monthly_count", 0)
            )

            # Calculate percentages
            hourly_used_pct = (
                (usage_counts.get("hourly_count", 0) / config_rule.hourly_limit * 100)
                if config_rule.hourly_limit > 0
                else 0
            )
            daily_used_pct = (
                (usage_counts.get("daily_count", 0) / config_rule.daily_limit * 100)
                if config_rule.daily_limit > 0
                else 0
            )
            monthly_used_pct = (
                (usage_counts.get("monthly_count", 0) / config_rule.monthly_limit * 100)
