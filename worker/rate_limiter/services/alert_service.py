...
class RateLimitAlertService:
    ...
    def start_monitoring(self):
        ...
        self.monitoring_thread = threading.Thread(
            target=self._monitoring_loop, name="rate_limit_alert_monitor", daemon=True
        )
        ...
        try:
            ...
            for config_rule in active_configs:
                ...
                try:
                    ...
                    logger.warning(
                        f"Error checking entity {config_rule.entity_type}:"
                        f"{config_rule.identifier}: {e}"
                    )
                    continue

            logger.debug(
                f"Monitoring cycle: checked {entities_checked} entities, "
    ...
    def check_thresholds(
        self, organization_id: str, domain: str, email: str, direction: str
    ) -> bool:
        """
            ...
            for entity_type, identifier in entities_to_check:
                try:
                    ...
                    for window_type, usage_percentage, limit_value in time_windows:
                        ...
                        alert_level = self._determine_alert_level(usage_percentage, config_rule)
                    ...
                    logger.warning(f"Error checking thresholds for {entity_type}:{identifier}: {e}")
                    continue
    ...
    def _should_send_alert(
        self, entity_type: str, identifier: str, direction: str, alert_level: str, window_type: str
    ) -> bool:
        """
    ...
    def _cleanup_old_alerts(self):
        ...
        # Remove old entries
        keys_to_remove = [
            key for key, timestamp in self.alert_history.items() if timestamp < cutoff_time
        ]
    ...
    def _send_threshold_alert(
        ...
        try:
            ...
            alert_payload = {
                ...
                "entity": {"type": entity_type, "identifier": identifier, "direction": direction},
                "threshold": {
            ...
            if alert_stored:
                self.alerts_sent += 1
                logger.info(
                    f"Sent {alert_level} alert for {entity_type}:{identifier} "
    ...
    def send_manual_alert(
        ...
        try:
            ...
            # Store manual alert
            alert_stored = self.usage_service.database_service.create_alert_record(
                entity_type, identifier, direction, alert_level, 0, 0, "manual"
            )
    ...
    def cleanup_old_alerts(self, retention_days: int = 30) -> int:
        ...
        try:
            ...
            old_alerts = [
                key for key, timestamp in self.alert_history.items() if timestamp < cutoff_date
            ]
