...
class RateLimitCacheService:
    ...
    def _init_redis(self) -> Optional[redis.Redis]:
        ...
        try:
            ...
            redis_client = redis.Redis(connection_pool=redis.ConnectionPool(**redis_kwargs))
    ...
    def increment_counter(
        ...
        try:
            ...
            hour_key = (
                f"rate:{entity_type}:{identifier}:{direction}:hour:{now.strftime('%Y-%m-%d-%H')}"
            )
            ...
            logger.debug(
                f"Incremented counters for {entity_type}:{identifier}:{direction} by {amount}"
            )
    ...
    def get_bulk_counts(self, entities: List[Tuple[str, str, str]]) -> Dict[str, Dict[str, int]]:
        ...
        try:
            ...
            for entity_type, identifier, direction in entities:
                ...
                day_key = (
                    f"rate:{entity_type}:{identifier}:{direction}:day:{now.strftime('%Y-%m-%d')}"
                )
                month_key = (
                    f"rate:{entity_type}:{identifier}:{direction}:month:{now.strftime('%Y-%m')}"
                )
    ...
    def reset_counters(self, entity_type: str, identifier: str, direction: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Reset {deleted_count} counters for {entity_type}:{identifier}:{direction}"
            )
            return True
    ...
    def cleanup_expired_counters(self) -> int:
        ...
        try:
            ...
            patterns = ["rate:*:hour:*", "rate:*:day:*", "rate:*:month:*"]
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        """
        health = {"service": "rate_limit_cache", "status": "healthy", "checks": {}}
