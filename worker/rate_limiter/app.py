...
class EnterpriseRateLimiter:
    ...
    def _run_cleanup_service(self):
        ...
        while True:
            try:
                ...
                self.alert_service.cleanup_old_alerts(
                    retention_days=config.cleanup_retention_days
                )
            ...
            with LogTimer(log_performance, f"rate_limit_check_{direction}"):
                ...
                for entity_type, identifier in checks:
                    ...
                    if not allowed:
                        ...
                        return False, f"{entity_type.title()} rate limit exceeded: {message}", details
    ...
    def _check_single_entity_limit(self, entity_type: str, identifier: str, 
        ...
        try:
            ...
            usage_stats = self.usage_service.get_current_usage(
                entity_type, identifier, direction
            )
            ...
            if config_rule.daily_limit > 0:
                percentages['daily'] = (
                    usage_stats['daily_count'] / config_rule.daily_limit
                ) * 100
            ...
            if config_rule.second_limit > 0 and usage_stats['second_count'] >= config_rule.second_limit:
                dispatch_event(Events.RATE_LIMIT_EXCEEDED, data={
                    'entity_type': entity_type, 'identifier': identifier,
                    'direction': direction, 'window': 'second',
                ...
                return False, f"Per-second limit ({config_rule.second_limit}) exceeded - anti-spam protection", check_details

            if config_rule.minute_limit > 0 and usage_stats['minute_count'] >= config_rule.minute_limit:
                dispatch_event(Events.RATE_LIMIT_EXCEEDED, data={
                    'entity_type': entity_type, 'identifier': identifier,
                    'direction': direction, 'window': 'minute',
                   ...
                   domain=identifier if entity_type == 'domain' else None,
                   source_service="rate_limiter")
                return False, f"Per-minute limit ({config_rule.minute_limit}) exceeded - burst protection", check_details

            if config_rule.hourly_limit > 0 and usage_stats['hourly_count'] >= config_rule.hourly_limit:
                dispatch_event(Events.RATE_LIMIT_EXCEEDED, data={
                    'entity_type': entity_type, 'identifier': identifier,
                    'direction': direction, 'window': 'hourly',
            ...
            if config_rule.daily_limit > 0 and usage_stats['daily_count'] >= config_rule.daily_limit:
                dispatch_event(Events.RATE_LIMIT_EXCEEDED, data={
                    'entity_type': entity_type, 'identifier': identifier,
                    'direction': direction, 'window': 'daily',
            ...
            if config_rule.monthly_limit > 0 and usage_stats['monthly_count'] >= config_rule.monthly_limit:
                dispatch_event(Events.RATE_LIMIT_EXCEEDED, data={
                    'entity_type': entity_type, 'identifier': identifier,
                    'direction': direction, 'window': 'monthly',
            ...
            for window_name, count_key, limit_val in [
                ...
                if limit_val > 0:
                    ...
                    if pct >= config_rule.warning_threshold:
                        dispatch_event(Events.RATE_LIMIT_THRESHOLD, data={
                            'entity_type': entity_type, 'identifier': identifier,
                            'direction': direction, 'window': window_name,
                            'current_count': usage_stats[count_key],
                            'limit': limit_val,
                            'usage_percentage': round(pct, 2),
                            'threshold_level': 'critical' if pct >= config_rule.critical_threshold else 'warning'
                        }, org_id=identifier if entity_type == 'organization' else None,
                           domain=identifier if entity_type == 'domain' else None,
                           source_service="rate_limiter")
    ...
    def increment_usage(self, email: str, direction: str, amount: int = 1) -> bool:
        ...
        try:
            with LogTimer(log_performance, f"usage_increment_{direction}"):
                ...
                if success:
                    self.alert_service.check_thresholds(
                        organization_id, domain, email, direction
                    )
    ...
    def get_usage_statistics(self, entity_type: str, identifier: str, 
        ...
        try:
            ...
            if config_rule.hourly_limit > 0:
                percentages['hourly'] = (usage_stats['hourly_count'] / config_rule.hourly_limit) * 100
            ...
            if config_rule.monthly_limit > 0:
                percentages['monthly'] = (usage_stats['monthly_count'] / config_rule.monthly_limit) * 100
                remaining['monthly'] = max(0, config_rule.monthly_limit - usage_stats['monthly_count'])
...
async def set_limits(request: Request):
    ...
    try:
        ...
        if success:
            dispatch_event(Events.RATE_LIMIT_RESET, data={
                'entity_type': data['type'],
                'identifier': data['identifier'],
                'direction': data['direction'],
                'hourly_limit': data.get('hourly_limit', 0),
