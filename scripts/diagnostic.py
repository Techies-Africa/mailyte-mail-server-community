...
class MailServerDiagnostic:
    ...
    def check_docker_environment(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.report['docker_compose'] = {'installed': True, 'version': result.stdout.strip()}
            else:
    ...
    def check_service_structure(self):
        ...
        services = {
            ...
            'worker': [
                'api', 'tracking', 'webhooks', 'rate_limiter', 'analytics',
                'monitoring', 'rag', 'storage_usage', 'queue_manager',
                'archiver', 'activesync', 'encryption', 'dashboard',
                'delivery_optimizer', 'cloud_sync'
            ]
        }
    ...
    def check_critical_paths(self):
        ...
        critical_dirs = [
            'storage', 'logs', 'config', 'shared', 'database',
            'storage/mail_data', 'storage/ssl_certs', 'storage/attachments'
        ]
