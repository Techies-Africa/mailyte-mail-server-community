...
#     peekride.com (Outlook) and talkam.net (Zoho) have ZERO mailboxes; their
#     mail is not ours and changing their MX would break it.
#   * Only domains that currently resolve. wusobtech.com and zinghunt.com
...
DOMAINS=(
  ...
  reapwork.com        # 12
  luminarylabs.ng     # 10
  devpilot.io         #  9  (publishes a STALE 1024-bit DKIM key from mailcow)
  kata.ng             #  4  (separate Cloudflare account -- needs its own token)
  holarscouture.com   #  2
  informly.app        #  2  (5 stale Namecheap eforward MX records still live)
  ogapethomes.com     #  2
  thatsign.ai         #  2
  ondgo.co            #  1
  tmahelp.ng          #  1  (already correct; included so it stays that way)
...
for domain in "${DOMAINS[@]}"; do
  ...
  # tmahelp.ng is on `courier`, and it is the one domain that currently passes
  ...
  # informly.app still lists five Namecheap eforward hosts at equal or lower
