...
class CertificateManager:
    ...
    def obtain_wildcard_certificate(self):
        ...
        cmd = [
            ...
            '-d', self.wildcard_domain,
            '-d', f'*.{self.wildcard_domain}',
        ]
        if self.staging:
            cmd.append('--staging')
        ...
        try:
            ...
            logger.info(f"Wildcard cert deployed as server.crt/server.key for *.{self.wildcard_domain}")
    ...
    def _should_use_wildcard(self, domain):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT COUNT(*) FROM domains WHERE domain LIKE %s AND active = 1",
                (f"%.{domain}",)
            )
    ...
    def process_domains(self):
        ...
        logger.info(
            f"Checking {len(groups)} domain groups ({len(domains)} domains total)"
        )
        ...
        with ThreadPoolExecutor(max_workers=self.cert_worker_threads) as pool:
            futures = {
                pool.submit(_process_group, item): item[0]
                for item in groups_needing_renewal
            }
