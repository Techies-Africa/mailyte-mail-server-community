...
class BounceHandler:
    ...
    def process_bounce(self, email_content: str):
        ...
        try:
            ...
            event_type = Events.DELIVERY_BOUNCE_HARD if bounce_type == 'HARD' else Events.DELIVERY_BOUNCE_SOFT
            dispatch_event(
                event_type,
                data={
                    "recipient": original_recipient,
    ...
    def _report_bounce(self, bounce_data: dict):
        ...
        try:
            response = requests.post(
                f"{self.tracking_service_url}/api/tracking/bounce",
                json=bounce_data,
                timeout=10
            )
    ...
    def _send_bounce_webhook(self, bounce_data: dict):
        ...
        try:
            ...
            response = requests.post(
                f"{self.webhook_service_url}/webhook/send",
                json=webhook_data,
                timeout=10
            )
            response.raise_for_status()
