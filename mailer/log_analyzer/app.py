...
class MailLogAnalyzer:
    ...
    def __init__(self):
        ...
        self.patterns = {
            "postfix_sent": re.compile(
                r"postfix/smtp\[\d+\]: ([A-F0-9]+): to=<([^>]+)>, relay=([^,]+), .* status=sent"
            ),
            "postfix_bounced": re.compile(
                r"postfix/smtp\[\d+\]: ([A-F0-9]+): to=<([^>]+)>, relay=([^,]+), .* status=bounced"
            ),
            "postfix_deferred": re.compile(
                r"postfix/smtp\[\d+\]: ([A-F0-9]+): to=<([^>]+)>, relay=([^,]+), .* status=deferred"
            ),
            "postfix_rejected": re.compile(
                r"postfix/smtpd\[\d+\]: NOQUEUE: reject: .* from ([^[]+)\[([^\]]+)\]"
            ),
            "dovecot_login": re.compile(
                r"dovecot: imap-login: Login: user=<([^>]+)>, method=\w+, rip=([^,]+)"
            ),
            "dovecot_failed": re.compile(
                r"dovecot: auth-worker\(\d+\): sql\([^,]+,[^,]+\): Password mismatch for ([^,]+)"
            ),
            "rspamd_spam": re.compile(
                r"rspamd\[\d+\]: <[^>]+>; task; spam: \w+ \[([0-9.]+)/15.00\]"
            ),
            "rspamd_ham": re.compile(
                r"rspamd\[\d+\]: <[^>]+>; task; ham: \w+ \[([0-9.-]+)/15.00\]"
            ),
        }
    ...
    def generate_summary_report(self, hours=24):
        ...
        total_attempts = (
            postfix_stats["sent"] + postfix_stats["bounced"] + postfix_stats["deferred"]
        )
        ...
        spam_rate = (
            (rspamd_stats["spam_detected"] / total_processed * 100) if total_processed > 0 else 0
        )
