...
class IntegrationTester:
    ...
    def test_tracking_functionality(self):
        ...
        try:
            ...
            if response.status_code == 200 and 'image' in response.headers.get('content-type', ''):
                ...
                return {'status': 'partial', 'message': 'Tracking endpoint responsive but unexpected content'}
    ...
    def run_all_tests(self):
        ...
        healthy_services = sum(1 for result in health_results.values() if result['status'] == 'pass')
        
        print(f"Service Health: {healthy_services}/{total_services} services healthy")
        print(f"API Tests: {sum(1 for test in api_tests if test['status'] == 'pass')}/{len(api_tests)} passed")
        ...
        print(f"Communication Tests: {sum(1 for test in comm_tests if test['status'] == 'pass')}/{len(comm_tests)} passed")
