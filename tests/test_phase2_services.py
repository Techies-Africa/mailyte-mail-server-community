...
def test_archiver_service():
    ...
    start = time.time()
    status, body = http_request(
        f"{ARCHIVER_URL}/archive/search?organization_id={TEST_ORG_ID}&q=Test"
    )
    r.duration = time.time() - start
    ...
    start = time.time()
    status, body = http_request(
        f"{ARCHIVER_URL}/retention-policy",
        "POST",
        {
            "organization_id": TEST_ORG_ID,
            "retention_days": 365,
...
def test_delivery_optimizer():
    ...
    if status == 200 and "send_now" in body:
        ...
        r.message = (
            f"{action} (hourly: {body.get('hourly_count', 0)}/{body.get('hourly_limit', 0)})"
        )
    else:
        r.message = f"HTTP {status}: {body}"
    results.append(r)
    ...
    start = time.time()
    status, body = http_request(
        f"{DELIVERY_URL}/warming/schedule",
        "POST",
        {
...
def test_templates_service():
    ...
    if template_id:
        ...
        start = time.time()
        status, body = http_request(
            f"{TEMPLATES_URL}/templates/{template_id}/render",
            "POST",
            {
        ...
        if (
            status == 200
            and body.get("subject") == "Hello John!"
            and "Hello John" in body.get("html", "")
        ):
            r.passed = True
...
def test_url_protection():
    ...
    start = time.time()
    status, body = http_request(
        f"{URL_PROTECTION_URL}/blocklist",
        "POST",
        {
            "organization_id": TEST_ORG_ID,
...
def test_api_routes():
    ...
    start = time.time()
    status, body = http_request(
        f"{API_URL}/api/v1/transport-rules/",
        "POST",
        {
            ...
            "condition_logic": "all",
            "actions": [
                {"type": "reject", "params": {"message": "Executable attachments not allowed"}}
            ],
            "priority": 10,
            "enabled": True,
        },
    ...
    start = time.time()
    status, body = http_request(
        f"{API_URL}/api/v1/message-trace/audit-logs?organization_id={TEST_ORG_ID}"
    )
    r.duration = time.time() - start
    if status == 200 and isinstance(body, list):
        r.passed = True
...
def setup_test_data():
    ...
    try:
        ...
        print(
            "mysql-connector-python required for setup. Install: pip install mysql-connector-python"
        )
        return False
