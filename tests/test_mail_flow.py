...
def test_auth_results(parsed_email):
    ...
    try:
        ...
        if auth_results:
            ...
            result.message = 'No Authentication-Results header — milter_headers may not be configured'
    except Exception as e:
        result.message = f'Check failed: {e}'
    result.duration = time.time() - start
    return result
...
def setup_test_data():
    ...
    try:
        ...
        print("mysql-connector-python not installed. Install with: pip3 install mysql-connector-python")
        return False
