...
class WebhookURL(Base):
    ...
    event_types = Column(JSON, nullable=False)  # Array: ['email.opened', 'email.clicked', 'smtp.delivered']
...
class WebhookDeliveryLog(Base):
    ...
    id = Column(String(26), primary_key=True, default=generate_ulid)
    webhook_url_id = Column(String(26), ForeignKey('webhook_urls.id'), nullable=True, index=True)  # NULL for ad-hoc webhooks
    ...
    delivery_status = Column(SQLEnum(WebhookDeliveryStatus), nullable=False, default=WebhookDeliveryStatus.PENDING, index=True)
    ...
    backoff_multiplier = Column(Float, nullable=False, default=2.0)  # Exponential backoff multiplier
